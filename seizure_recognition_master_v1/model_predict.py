...
def make_list(test_name, day_dir):
    ...
    # intermediate_dir = os.path.join(day_dir, 'move')
    ...
    test_file = os.path.join(day_dir, test_name)
    file = open(test_file, 'w')
    for root, dirs, files in os.walk(day_dir, topdown=False):
        files = sorted(files)
        for name in files:
            line = os.path.join(root, name) + ' #0'
            file.write(line)
            file.write('\n')
    return test_file
    ...
    with torch.no_grad():
        ...
        for i, (clip, targets, video_name) in enumerate(test_data_loader):
            ...
            if targets.item() == 0:
                ...
                if pre_label > 1:
                    for h in range(10):
                        if ((((prob_outputs.cpu().data.numpy())[h][1]) - (
                                (prob_outputs.cpu().data.numpy())[h][0]) > 0) and (
                                abs(((prob_outputs.cpu().data.numpy())[h][1]) - (
                                        (prob_outputs.cpu().data.numpy())[h][0])) < 0.3)):
