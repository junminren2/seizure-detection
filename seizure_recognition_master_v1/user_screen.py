...
def rename_video(kind_dir, exper, Num):
    prefix, day = os.path.split(kind_dir)
    kinds = os.listdir(kind_dir)
    for kind in kinds:
        day_dir = os.path.join(kind_dir, kind)
        ...
        for elem in elems:
            ...
            for name in sample:
                ...
                os.rename(v_path, os.path.join(h_dir, exper + '_' + Num + '_' + kind + '_' + elem + '_' + name))


def extract_move_video(kind_dir):
    # diffs = []
    kinds = os.listdir(kind_dir)
    for kind in kinds:
        day_dir = os.path.join(kind_dir, kind)
